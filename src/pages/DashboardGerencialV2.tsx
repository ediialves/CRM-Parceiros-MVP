...
// ============================================================
// Evolução por Intervenção — helpers de gráfico (Chart.js)
// ============================================================
const EI_COLORS = { playbook: '#4f46e5', bau: '#b45309', semplano: '#94a3b8' };
const eiFmtPct = (v: number) => `${v.toFixed(1).replace('.', ',')}%`;
const eiFmtN = (v: number) => Math.round(v).toLocaleString('pt-BR');

function eiChartData(ei: any, metricKey: 'pct' | 'lic' | 'licEng', weeksLabels: string[]) {
  return {
    labels: weeksLabels,
    datasets: [
      {
        label: ei.semPlano.label,
        data: ei.semPlano.series.map((s: any) => s[metricKey]),
        borderColor: EI_COLORS.semplano,
        backgroundColor: EI_COLORS.semplano,
        borderDash: [6, 4],
        pointRadius: 2,
        borderWidth: 2,
        tension: 0.3,
      ...
      {
        label: ei.bau.label,
        data: ei.bau.series.map((s: any) => s[metricKey]),
        borderColor: EI_COLORS.bau,
        backgroundColor: EI_COLORS.bau,
        pointRadius: 2,
        borderWidth: 2.4,
        tension: 0.3,
        spanGaps: true
      ...
      {
        label: ei.playbook.label,
        data: ei.playbook.series.map((s: any) => s[metricKey]),
        borderColor: EI_COLORS.playbook,
...
function eiChartOptions(fmt: (v: number) => string): any {
  return {
    responsive: true,
    ...
    plugins: {
      ...
      tooltip: {
        callbacks: {
          label: (ctx: any) => `${ctx.dataset.label}: ${ctx.parsed.y != null ? fmt(ctx.parsed.y) : '—'}`
        }
      }
    },
    scales: {
      ...
      y: {
        ...
        ticks: { color: '#94a3b8', font: { size: 10.5, family: 'Inter, sans-serif' }, callback: (v: any) => fmt(Number(v)) },
        grace: '15%'
      }
    }
  };
...
// Rótulos densos: playbook (dataset 2) sempre acima do próprio ponto, BAU (dataset 1) sempre do
// lado oposto ao playbook naquele índice (evita colisão mesmo quando as linhas se cruzam), Sem
// Plano (dataset 0) só no último ponto. No último índice as 3 séries podem estar em qualquer
// ordem, então são empilhadas de cima pra baixo pela posição real, com espaçamento mínimo.
function eiLabelsPlugin(fmt: (v: number) => string, maxWeeks: number) {
  const step = Math.max(1, Math.round(maxWeeks / 4));
  const LABEL_IDX = Array.from({ length: Math.floor(maxWeeks / step) }, (_, i) => i * step);
  return {
    id: 'ei-labels-' + fmt.name,
    afterDatasetsDraw(chart: any) {
      const { ctx } = chart;
      ctx.save();
      ctx.textAlign = 'center';

      const metaSemPlano = chart.getDatasetMeta(0);
      const metaBau = chart.getDatasetMeta(1);
      const metaPlaybook = chart.getDatasetMeta(2);
      const dataSemPlano = chart.data.datasets[0].data;
      const dataBau = chart.data.datasets[1].data;
      const dataPlaybook = chart.data.datasets[2].data;
      const n = dataPlaybook.length;

      function drawLabel(x: number, y: number, color: string, text: string, bold: boolean) {
        ctx.font = (bold ? '800 11px' : '700 9px') + ' Inter, sans-serif';
        ctx.lineWidth = 3;
        ctx.strokeStyle = '#fff';
      ...
      LABEL_IDX.forEach((i) => {
        if (i >= n - 1) return;
        const pv = dataPlaybook[i], bv = dataBau[i];
        if (pv == null || bv == null) return;
        const pointP = metaPlaybook.data[i], pointB = metaBau.data[i];
        if (!pointP || !pointB) return;
        const pAbove = pv >= bv;
        drawLabel(pointP.x, pointP.y + (pAbove ? -10 : 16), EI_COLORS.playbook, fmt(pv), false);
        drawLabel(pointB.x, pointB.y + (pAbove ? 16 : -10), EI_COLORS.bau, fmt(bv), false);
      });

      const lastIdx = n - 1;
      const entries: { color: string; val: number; y: number; x: number }[] = [];
      if (dataPlaybook[lastIdx] != null && metaPlaybook.data[lastIdx]) entries.push({ color: EI_COLORS.playbook, val: dataPlaybook[lastIdx], y: metaPlaybook.data[lastIdx].y, x: metaPlaybook.data[lastIdx].x });
      if (dataBau[lastIdx] != null && metaBau.data[lastIdx]) entries.push({ color: EI_COLORS.bau, val: dataBau[lastIdx], y: metaBau.data[lastIdx].y, x: metaBau.data[lastIdx].x });
      if (dataSemPlano[lastIdx] != null && metaSemPlano.data[lastIdx]) entries.push({ color: EI_COLORS.semplano, val: dataSemPlano[lastIdx], y: metaSemPlano.data[lastIdx].y, x: metaSemPlano.data[lastIdx].x });
      entries.sort((a, b) => a.y - b.y);
      const minGap = 14;
      const placed: number[] = [];
      entries.forEach((e, idx) => {
        let target = e.y - 5;
        if (idx > 0 && target < placed[idx - 1] + minGap) target = placed[idx - 1] + minGap;
        placed.push(target);
      });
      entries.forEach((e, idx) => drawLabel(e.x, placed[idx], e.color, fmt(e.val), true));
...
export const DashboardGerencialV2: React.FC = () => {
  ...
  // Evolução por Intervenção — filtros locais (independentes dos filtros globais da página)
  const [eiPlaybookId, setEiPlaybookId] = useState<string>('todos');
  const [eiSegmento, setEiSegmento] = useState<string>('todos');
  const [eiFila, setEiFila] = useState<'todos' | 'RETENÇÃO' | 'EXPANSÃO'>('todos');
  ...
  // ============================================================
  // Evolução por Intervenção — Playbook específico vs. BAU (plano manual) vs. Sem Plano,
  // alinhados por semanas desde a entrada (Playbook/BAU) ou por calendário real (Sem Plano,
  // que não tem uma data de "entrada"). Segmento é travado no snapshot mais próximo da data
  // de criação do plano/playbook — mesmo critério de getPartnerPlanoAtS0 usado no resto do
  // dashboard — para não distorcer o recorte com mudanças de segmento pós-intervenção.
  // ============================================================
  const evolucaoIntervencaoData = useMemo(() => {
    if (!rawData) return { status: 'loading' as const };
    const { partners = [], basePlans = [], partnerSnapshots = [] } = rawData;
    if (partners.length === 0) return { status: 'empty' as const };
    ...
    const partnerMap = new Map<string, any>();
    partners.forEach((p: any) => { if (p?.id) partnerMap.set(p.id, p); });

    type Snap = { dateStr: string; licencas: number; licencasEngajadas: number; plano: string | null };
    ...
    (partnerSnapshots || []).forEach((ps: any) => {
      ...
      list.push({
        dateStr,
        licencas: Number(ps.licencas) || 0,
        licencasEngajadas: Number(ps.licencas_engajadas) || 0,
        plano: ps.plano ? String(ps.plano).trim() : null
      });
      snapshotsByPartner.set(ps.partner_id, list);
    });
    snapshotsByPartner.forEach((list) => list.sort((a, b) => a.dateStr.localeCompare(b.dateStr)));

    const getLockedSegmento = (partnerId: string, dataRef: string): string | null => {
      const snaps = snapshotsByPartner.get(partnerId);
      if (!snaps || snaps.length === 0) return null;
      for (let i = snaps.length - 1; i >= 0; i--) {
        if (snaps[i].dateStr <= dataRef) return snaps[i].plano;
      }
      return null;
    };
    const getEngAt = (partnerId: string, dataRef: string): { pct: number; lic: number; licEng: number } | null => {
      const snaps = snapshotsByPartner.get(partnerId);
      if (!snaps || snaps.length === 0) return null;
      let found: Snap | null = null;
      for (let i = snaps.length - 1; i >= 0; i--) {
        if (snaps[i].dateStr <= dataRef) { found = snaps[i]; break; }
      }
      if (!found) return null;
      return { pct: found.licencas > 0 ? (found.licencasEngajadas / found.licencas) * 100 : 0, lic: found.licencas, licEng: found.licencasEngajadas };
    };
    const getLatestEng = (partnerId: string) => {
      const snaps = snapshotsByPartner.get(partnerId);
      if (!snaps || snaps.length === 0) return null;
      const last = snaps[snaps.length - 1];
      return { pct: last.licencas > 0 ? (last.licencasEngajadas / last.licencas) * 100 : 0 };
    };
    const getEarliestEng = (partnerId: string) => {
      const snaps = snapshotsByPartner.get(partnerId);
      if (!snaps || snaps.length === 0) return null;
      const first = snaps[0];
      return { pct: first.licencas > 0 ? (first.licencasEngajadas / first.licencas) * 100 : 0 };
    };

    type PlanEntry = { partnerId: string; entryDate: string; playbookId: string | null };
    const allPlanEntries: PlanEntry[] = [];
    (basePlans || []).forEach((p: any) => {
      if (!p?.partner_id || !p.created_at) return;
      if (!partnerMap.has(p.partner_id)) return;
      const entryDate = typeof p.created_at === 'string' ? p.created_at.split('T')[0] : String(p.created_at).substring(0, 10);
      if (!entryDate) return;
      allPlanEntries.push({ partnerId: p.partner_id, entryDate, playbookId: p.playbook_id || null });
    });
    const partnersWithAnyPlan = new Set(allPlanEntries.map((e) => e.partnerId));

    const earliestEntryFor = (entries: PlanEntry[]): PlanEntry => entries.reduce((a, b) => (a.entryDate <= b.entryDate ? a : b));

    const byPartnerPlaybookEntries = new Map<string, PlanEntry[]>();
    const byPartnerBauEntries = new Map<string, PlanEntry[]>();
    allPlanEntries.forEach((e) => {
      const matchesSelectedPlaybook = !!e.playbookId && (eiPlaybookId === 'todos' || e.playbookId === eiPlaybookId);
      if (matchesSelectedPlaybook) {
        const list = byPartnerPlaybookEntries.get(e.partnerId) || [];
        list.push(e);
        byPartnerPlaybookEntries.set(e.partnerId, list);
      }
      if (!e.playbookId) {
        const list = byPartnerBauEntries.get(e.partnerId) || [];
        list.push(e);
        byPartnerBauEntries.set(e.partnerId, list);
      }
    });

    const passesProfile = (partnerId: string, dataRef: string): boolean => {
      const partner = partnerMap.get(partnerId);
      if (!partner) return false;
      if (eiFila !== 'todos' && partner.fila !== eiFila) return false;
      if (eiSegmento !== 'todos') {
        const seg = getLockedSegmento(partnerId, dataRef);
        if (!seg || seg.toUpperCase() !== eiSegmento.toUpperCase()) return false;
      }
      return true;
    };
    ...
    function buildGroupSeries(partnerIds: string[], entryDateByPartner: Map<string, string>) {
      const weeks = Array.from({ length: MAX_WEEKS + 1 }, (_, w) => w);
      return weeks.map((w) => {
        let sumLic = 0, sumLicEng = 0, count = 0;
        partnerIds.forEach((pid) => {
          const entryDate = entryDateByPartner.get(pid)!;
          const refDate = addDays(entryDate, w * 7 + 6);
          const at = getEngAt(pid, refDate);
          if (at) { sumLic += at.lic; sumLicEng += at.licEng; count++; }
        });
        const pct = sumLic > 0 ? (sumLicEng / sumLic) * 100 : null;
        return { week: w, pct, lic: sumLic > 0 ? sumLic : null, licEng: sumLic > 0 ? sumLicEng : null, count };
      });
    }

    function buildCalendarSeries(partnerIds: string[]) {
      let minDate: string | null = null;
      partnerIds.forEach((pid) => {
        const snaps = snapshotsByPartner.get(pid);
        if (snaps && snaps.length > 0 && (!minDate || snaps[0].dateStr < minDate)) minDate = snaps[0].dateStr;
      });
      if (!minDate) return [];
      const startMonday = getMondayOfWeek(minDate);
      const weeks = Array.from({ length: MAX_WEEKS + 1 }, (_, w) => w);
      return weeks.map((w) => {
        const refDate = addDays(startMonday, w * 7 + 6);
        let sumLic = 0, sumLicEng = 0, count = 0;
        partnerIds.forEach((pid) => {
          const at = getEngAt(pid, refDate);
          if (at) { sumLic += at.lic; sumLicEng += at.licEng; count++; }
    ...
    function outcomeCounts(partnerIds: string[], baselineFn: (pid: string) => number | null) {
      let evoluiu = 0, manteve = 0, regrediu = 0;
      partnerIds.forEach((pid) => {
        const base = baselineFn(pid);
        const latest = getLatestEng(pid);
        if (base === null || !latest) return;
        const delta = latest.pct - base;
        if (delta >= 3) evoluiu++;
        else if (delta <= -3) regrediu++;
        else manteve++;
      });
      return { evoluiu, manteve, regrediu };
    }

    const playbookPartnerIds = Array.from(byPartnerPlaybookEntries.keys()).filter((pid) => {
      const entry = earliestEntryFor(byPartnerPlaybookEntries.get(pid)!);
      return passesProfile(pid, addDays(entry.entryDate, 6));
    });
    const playbookEntryDates = new Map(playbookPartnerIds.map((pid) => [pid, earliestEntryFor(byPartnerPlaybookEntries.get(pid)!).entryDate]));

    const bauPartnerIds = Array.from(byPartnerBauEntries.keys()).filter((pid) => {
      const entry = earliestEntryFor(byPartnerBauEntries.get(pid)!);
      return passesProfile(pid, addDays(entry.entryDate, 6));
    });
    const bauEntryDates = new Map(bauPartnerIds.map((pid) => [pid, earliestEntryFor(byPartnerBauEntries.get(pid)!).entryDate]));

    // Sem Plano: nunca teve nenhum plano historicamente. Não existe uma data de "entrada" pra
    // travar segmento, então usa o segmento/fila atuais do parceiro como perfil de referência.
    const semPlanoPartnerIds = partners
      .map((p: any) => p.id)
      .filter((id: string) => id && !partnersWithAnyPlan.has(id))
      .filter((id: string) => {
        const partner = partnerMap.get(id);
        if (!partner) return false;
        if (eiFila !== 'todos' && partner.fila !== eiFila) return false;
        if (eiSegmento !== 'todos' && (!partner.segmento || partner.segmento.trim().toUpperCase() !== eiSegmento.toUpperCase())) return false;
        return true;
      });

    if (playbookPartnerIds.length === 0 && bauPartnerIds.length === 0 && semPlanoPartnerIds.length === 0) {
      return { status: 'empty' as const };
    }

    const playbookSeries = buildGroupSeries(playbookPartnerIds, playbookEntryDates);
    const bauSeries = buildGroupSeries(bauPartnerIds, bauEntryDates);
    const semPlanoSeries = buildCalendarSeries(semPlanoPartnerIds);

    const playbookOutcome = outcomeCounts(playbookPartnerIds, (pid) => {
      const at = getEngAt(pid, addDays(playbookEntryDates.get(pid)!, 6));
      return at ? at.pct : null;
    });
    const bauOutcome = outcomeCounts(bauPartnerIds, (pid) => {
      const at = getEngAt(pid, addDays(bauEntryDates.get(pid)!, 6));
      return at ? at.pct : null;
    });
    const semPlanoOutcome = outcomeCounts(semPlanoPartnerIds, (pid) => {
      const e = getEarliestEng(pid);
      return e ? e.pct : null;
    });

    const playbookLabel = eiPlaybookId === 'todos' ? 'Todos os playbooks' : (playbooksList.find((pb) => pb.id === eiPlaybookId)?.nome || 'Playbook');

    return {
      status: 'ok' as const,
      maxWeeks: MAX_WEEKS,
      playbook: { label: playbookLabel, n: playbookPartnerIds.length, series: playbookSeries, outcome: playbookOutcome },
      bau: { label: 'BAU (plano manual)', n: bauPartnerIds.length, series: bauSeries, outcome: bauOutcome },
      semPlano: { label: 'Sem Plano (referência)', n: semPlanoPartnerIds.length, series: semPlanoSeries, outcome: semPlanoOutcome }
    };
  }, [rawData, eiPlaybookId, eiSegmento, eiFila, playbooksList]);
  ...
  return (
    <div className="min-h-screen bg-bg-primary p-6 md:p-8">
      ...
      <div className="flex flex-col gap-6">
        ...
        {/* Card: Evolução por Intervenção (Playbook específico x BAU x Sem Plano) */}
        <div className="mt-8 rounded-xl border border-border bg-card p-6 shadow-sm" id="card-evolucao-intervencao">
          <div className="mb-4">
            <h3 className="text-lg font-bold text-text-primary" id="title-evolucao-intervencao">
              Evolução por Intervenção
            </h3>
            <p className="text-xs text-text-secondary mt-0.5">
              Compare um playbook específico (ou o agregado de todos) contra BAU e contra nenhuma intervenção, recortando por perfil do parceiro.
            </p>
          </div>

          {/* Filtros locais */}
          <div className="flex flex-wrap items-center gap-3 bg-bg-secondary/20 border border-border/60 rounded-lg px-4 py-3 mb-3">
            <div className="flex items-center gap-2">
              <span className="text-[10px] font-bold uppercase tracking-wider text-text-secondary">Playbook</span>
              <select
                value={eiPlaybookId}
                onChange={(e) => setEiPlaybookId(e.target.value)}
                className="h-8 px-2.5 bg-card border border-border rounded-lg text-xs font-semibold text-text-primary focus:outline-none focus:ring-2 focus:ring-indigo-500 cursor-pointer"
              >
            ...
            <div className="flex items-center gap-1.5">
              <span className="text-[10px] font-bold uppercase tracking-wider text-text-secondary mr-1">Segmento</span>
              <button
                onClick={() => setEiSegmento('todos')}
                className={`text-[11px] font-semibold px-3 py-1 rounded-full border ${eiSegmento === 'todos' ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-card text-text-secondary border-border'}`}
              >Todos</button>
              {availablePlanos.map((seg) => (
                <button
                  key={seg}
                  onClick={() => setEiSegmento(seg)}
                  className={`text-[11px] font-semibold px-3 py-1 rounded-full border ${eiSegmento === seg ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-card text-text-secondary border-border'}`}
                >{seg}</button>
              ))}
            </div>
            <div className="w-px h-5 bg-border" />
            <div className="flex items-center gap-1.5">
              <span className="text-[10px] font-bold uppercase tracking-wider text-text-secondary mr-1">Fila</span>
              {(['todos', 'RETENÇÃO', 'EXPANSÃO'] as const).map((f) => (
                <button
                  key={f}
                  onClick={() => setEiFila(f)}
                  className={`text-[11px] font-semibold px-3 py-1 rounded-full border ${eiFila === f ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-card text-text-secondary border-border'}`}
                >{f === 'todos' ? 'Todas' : f.charAt(0) + f.slice(1).toLowerCase()}</button>
              ))}
            </div>
          </div>
          <p className="text-[11px] text-text-secondary mb-5 leading-relaxed">
            <b className="text-text-primary">Como ler:</b> "Playbook" e "BAU" são alinhados por semanas desde a entrada no plano (S0 = criação); "Sem Plano" é a referência do mesmo perfil, no mesmo período real de calendário — não tem "entrada", por isso não pode ser alinhado da mesma forma. O filtro de <b className="text-text-primary">Segmento</b> usa o valor travado no snapshot mais próximo da data de criação do plano/playbook (mesmo critério de <code>getPartnerPlanoAtS0</code> já usado no resto do dashboard) — para Sem Plano, que não tem essa data de referência, usa o segmento mais recente do parceiro.
          </p>

          {evolucaoIntervencaoData.status === 'loading' ? (
          ...
          ) : evolucaoIntervencaoData.status === 'empty' ? (
            <div className="py-12 text-center text-text-secondary bg-bg-secondary/20 rounded-lg p-6">
              <p className="font-semibold text-sm text-text-primary">Nenhum parceiro encontrado para esse recorte.</p>
              <p className="text-xs mt-1 text-text-secondary">Tente ajustar o playbook, segmento ou fila selecionados.</p>
            </div>
          ) : (() => {
            const ei = evolucaoIntervencaoData as any;
            const weeksLabels = Array.from({ length: ei.maxWeeks + 1 }, (_, w) => `S${w}`);
            const lastPct = (g: any) => {
              const vals = g.series.filter((s: any) => s.pct != null);
              return vals.length ? vals[vals.length - 1].pct : null;
            };
            const firstPct = (g: any) => {
              const vals = g.series.filter((s: any) => s.pct != null);
              return vals.length ? vals[0].pct : null;
            };
            const lastVal = (g: any, key: 'lic' | 'licEng') => {
              const vals = g.series.filter((s: any) => s[key] != null);
              return vals.length ? vals[vals.length - 1][key] : null;
            };

            const kpiCard = (g: any, colorClass: string, borderClass: string) => {
              const lp = lastPct(g), fp = firstPct(g);
              const dEng = lp != null && fp != null ? lp - fp : null;
              const licEngLast = lastVal(g, 'licEng');
              return (
                <div className={`relative overflow-hidden rounded-xl border ${borderClass} bg-bg-secondary/10 p-4`}>
                  <div className={`absolute top-0 left-0 right-0 h-1 ${colorClass}`} />
                  <div className="text-sm font-bold text-text-primary">{g.label}</div>
                  <div className="text-[11px] text-text-secondary">{g.n} parceiros</div>
                  <div className="flex gap-5 mt-3">
                    <div>
                      <div className="text-[9.5px] font-bold uppercase tracking-wider text-text-secondary">Engajamento</div>
                      <div className="text-lg font-extrabold text-text-primary">{lp != null ? eiFmtPct(lp) : '—'}</div>
                      {dEng != null && (
                        <div className={`text-[10.5px] font-bold ${dEng >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>
                          {dEng >= 0 ? '+' : ''}{dEng.toFixed(1).replace('.', ',')}p.p.
                        </div>
                      )}
                    </div>
                    <div>
                      <div className="text-[9.5px] font-bold uppercase tracking-wider text-text-secondary">Lic. engajadas</div>
                      <div className="text-lg font-extrabold text-text-primary">{licEngLast != null ? eiFmtN(licEngLast) : '—'}</div>
                    </div>
                  </div>
                </div>
              );
            ...
            const distRow = (g: any, colorClass: string) => {
              const total = g.outcome.evoluiu + g.outcome.manteve + g.outcome.regrediu;
              const pct = (v: number) => (total > 0 ? Math.round((v / total) * 100) : 0);
              return (
                <div className="flex items-center gap-3 mb-3 last:mb-0">
                  <div className="w-44 shrink-0">
                    <div className="text-xs font-bold text-text-primary">{g.label}</div>
                    <div className="text-[10px] text-text-secondary">{g.n} parceiros</div>
                  </div>
                  <div className="flex-1 flex h-7 rounded-lg overflow-hidden bg-bg-secondary/30">
                    {pct(g.outcome.evoluiu) > 0 && (
                      <div className="flex items-center justify-center text-[10.5px] font-extrabold text-white bg-emerald-600" style={{ width: `${pct(g.outcome.evoluiu)}%` }}>
                        {pct(g.outcome.evoluiu) >= 8 ? `${pct(g.outcome.evoluiu)}%` : ''}
                      </div>
                    )}
                    {pct(g.outcome.manteve) > 0 && (
                      <div className="flex items-center justify-center text-[10.5px] font-extrabold text-white bg-slate-400" style={{ width: `${pct(g.outcome.manteve)}%` }}>
                        {pct(g.outcome.manteve) >= 8 ? `${pct(g.outcome.manteve)}%` : ''}
                      </div>
                    )}
                    {pct(g.outcome.regrediu) > 0 && (
                      <div className="flex items-center justify-center text-[10.5px] font-extrabold text-white bg-rose-600" style={{ width: `${pct(g.outcome.regrediu)}%` }}>
                        {pct(g.outcome.regrediu) >= 8 ? `${pct(g.outcome.regrediu)}%` : ''}
                      </div>
                    )}
                  </div>
                </div>
            ...
            const lineCard = (title: string, sub: string, metricKey: 'pct' | 'lic' | 'licEng', fmt: (v: number) => string) => (
              <div className="rounded-xl border border-border/70 bg-bg-secondary/10 p-4">
                <div className="flex items-center justify-between mb-1 flex-wrap gap-2">
                  <div>
                    <div className="text-sm font-bold text-text-primary">{title}</div>
                    <div className="text-[11px] text-text-secondary">{sub}</div>
                  </div>
                  <div className="flex gap-4 text-[10.5px] font-semibold text-text-secondary">
                    <span className="flex items-center gap-1.5"><span className="w-3 h-0.5 rounded-full inline-block" style={{ background: EI_COLORS.playbook }} />{ei.playbook.label}</span>
                    <span className="flex items-center gap-1.5"><span className="w-3 h-0.5 rounded-full inline-block" style={{ background: EI_COLORS.bau }} />BAU</span>
                    <span className="flex items-center gap-1.5"><span className="w-3 h-0.5 rounded-full inline-block border-t border-dashed" style={{ borderColor: EI_COLORS.semplano }} />Sem Plano</span>
                  </div>
                </div>
                <div className="h-64 w-full mt-2">
                  <Line
                    data={eiChartData(ei, metricKey, weeksLabels)}
                    options={eiChartOptions(fmt)}
                    plugins={[eiLabelsPlugin(fmt, ei.maxWeeks)]}
                  />
                </div>
              </div>
            );
            ...
            return (
              <>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-6">
                  {kpiCard(ei.playbook, 'bg-indigo-600', 'border-border')}
                  {kpiCard(ei.bau, 'bg-amber-600', 'border-border')}
                  {kpiCard(ei.semPlano, 'bg-slate-400', 'border-border')}
                </div>

                <div className="rounded-xl border border-border/70 bg-bg-secondary/10 p-4 mb-6">
                  <div className="mb-3">
                    <div className="text-sm font-bold text-text-primary">Engajamento: S0 vs. hoje, por parceiro</div>
                    <div className="text-[11px] text-text-secondary">Cada parceiro classificado pela própria variação de engajamento — não é a média do grupo.</div>
                  </div>
                  {distRow(ei.playbook, 'bg-indigo-600')}
                  {distRow(ei.bau, 'bg-amber-600')}
                  {distRow(ei.semPlano, 'bg-slate-400')}
                  <div className="flex gap-4 mt-4 text-[10.5px] font-semibold text-text-secondary">
                    <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-sm inline-block bg-emerald-600" />Evoluiu (+3p.p. ou mais)</span>
                    <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-sm inline-block bg-slate-400" />Manteve (±3p.p.)</span>
                    <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-sm inline-block bg-rose-600" />Regrediu (-3p.p. ou mais)</span>
                  </div>
                </div>

                <div className="flex flex-col gap-4">
                  {lineCard('Engajamento (%)', 'Percentual de licenças engajadas, ponderado pelo volume do grupo', 'pct', eiFmtPct)}
                  {lineCard('Volume de licenças', 'Total de licenças ativas nos parceiros do grupo', 'lic', eiFmtN)}
                  {lineCard('Licenças engajadas', 'Total de licenças efetivamente engajadas', 'licEng', eiFmtN)}
                </div>
              </>
            );
          })()}
