...
- **Evolução por Intervenção** (seção do Dashboard Gerencial V2): compara 3 grupos de parceiros — **Playbook** (um playbook automático específico, ou o agregado de todos), **BAU** (plano manual, `playbook_id` nulo) e **Sem Plano** (nunca teve nenhum plano) — ao longo de semanas desde a entrada (S0..S12). Sem Plano não tem "entrada", então é alinhado por calendário real no mesmo período observado, não por semana relativa.
...
- **Evolução por Intervenção agrega por soma, não por média de percentuais**: cada ponto de cada grupo é `soma(licencas_engajadas) / soma(licencas) * 100` entre os parceiros do grupo naquela semana — mesmo critério de `cohortEngagementData` — não a média aritmética dos percentuais individuais (que dá peso igual a parceiros grandes e pequenos, distorcendo o resultado).
- **Evolução por Intervenção: "Evoluiu/Manteve/Regrediu" é por parceiro, não estimado**: compara o engajamento do parceiro em S0 (ou no snapshot mais antigo, para Sem Plano) contra a leitura mais recente disponível, com limiar de ±3 p.p. — mesmo threshold já usado no radar de queda de engajamento (ver seção 2, "Queda de engajamento").
- **Evolução por Intervenção: um parceiro pode contar em mais de um grupo**: se um parceiro já teve tanto um plano manual (BAU) quanto um playbook automático em momentos diferentes, ele entra nos dois grupos (cada grupo usa a entrada mais antiga daquele tipo específico) — não há exclusividade mútua entre os grupos, de propósito, já que a pergunta é "qual o efeito de cada tipo de intervenção", não "a que grupo pertence este parceiro".
...
- **Evolução por Intervenção não foi verificada visualmente com dados reais** (Claude Code, 2026-08-25): implementada e validada via `tsc --noEmit` + `npm run build` (ambos limpos) e espelhando os padrões já testados de `cohortEngagementData`, mas sem credenciais Supabase neste ambiente para rodar `npm run dev` e conferir os números contra a base real. Testar localmente ou após publicar via AI Studio antes de confiar nos resultados — atenção especial a: contagem de parceiros por grupo fazendo sentido, e a divisão Sem Plano (que usa segmento atual em vez de travado, por não ter data de entrada) não incluindo parceiros que na verdade já tiveram algum plano.
